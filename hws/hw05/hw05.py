...
def crispr_gene_insertion(lnk_of_genes, insert):
    ...
    """
    "*** YOUR CODE HERE ***"
    def inside(gene, index, insert):
        if gene.rest == Link.empty:
            return
        if gene.first == 'AUG':
            for i in range(index):
                aug = Link(insert, gene.rest)
                gene.rest = aug
        else:
            inside(gene.rest, index, insert)
    idx = 0
    while lnk_of_genes != Link.empty:
        inside(lnk_of_genes.first, idx + 1, insert)
