...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        pos, idx = (self.place, 0)
        while not pos.is_hive and idx <= self.upper_bound:
            if pos.bees and idx >= self.lower_bound:
...
class HungryAnt(Ant):
    ...
    name = 'Hungry'
    food_cost = 4
    chewing_turns = 3  # need 3 turns to chewing a bee
    implemented = True
    
    def __init__(self, health=1):
        self.turns_to_chew = 0  # turns remain to eat a bee
        super().__init__(health)
        
    def action(self, gamestate):
        while self.turns_to_chew > 0:
...
class ContainerAnt(Ant):
    ...
    def store_ant(self, ant):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        assert isinstance(ant, Ant), "{} is not an ant!".format(ant)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        def recur(place):
            if not isinstance(place, AntHomeBase):
                if place.ant:
                    place.ant.double()
                recur(place.exit)
            else:
                return
        recur(self.place.exit)
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def slow_action(gamestate):
            slow_action.turn_passed += 1
            if gamestate.time % 2 == 0 or slow_action.turn_passed > 5:
                return type(target).action(target, gamestate)
            else:
                return
        slow_action.turn_passed = 0
        if target:
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if target:
            target.scare(2)
            target.scared = True  # add an instance attr for bee to indicate it is scared
        super().throw_at(target)
        # END Problem EC 2
...
class LaserAnt(ThrowerAnt):
    ...
    def __init__(self, health=1):
        super().__init__(health)
        self.insects_shot = 0  # 记录已经击中的昆虫数量

    def insects_in_front(self):
        """返回一个字典，其中每个键是一个昆虫，每个对应的值是该昆虫距离LaserAnt的距离。"""
        insects_and_distances = {}
        current = self.place
        distance = 0

        while current is not None and not current.is_hive:
            if current.ant is not None and current.ant is not self:
                insects_and_distances[current.ant] = distance
            if current.bees:
...
class Bee(Insect):
    ...
    def scare(self, length):
        ...
        # BEGIN Problem EC 2
        "*** YOUR CODE HERE ***"
        if not hasattr(self, 'scared'):  # not scared yet
