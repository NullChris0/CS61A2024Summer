...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    diff = 3 * abs(player_score % 10 - opponent_score // 10 % 10)
    return max(diff, 1)
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    first_time = strategy(0, 0)
    for i in range(goal):
        for j in range(goal):
            if strategy(i, j) != first_time:
                return False
    return True
    # END PROBLEM 7
...
def make_averaged(original_function, samples_count=1000):
    ...
    def new_function(*args):
        ...
        for i in range(0, samples_count):
            d_all += original_function(*args)
        return d_all / samples_count
    return new_function
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    for num_rolls in range(1, 11):
        current =  make_averaged(roll_dice, samples_count)(num_rolls, dice)
        if current > expected:
            expected, index = current, num_rolls
    return index
    # END PROBLEM 9
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    n = randint(0, 3) if score - opponent_score > 20 or GOAL - score < 10 else 6
    # BEGIN PROBLEM 12
    if score - opponent_score < 10:
        return boar_strategy(score, opponent_score, num_rolls=n)
    else:
        return sus_strategy(score, opponent_score, num_rolls=n)
    # END PROBLEM 12
