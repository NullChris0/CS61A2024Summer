...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    para = list(filter(select, paragraphs))
    return para[k] if k < para.__len__() else ''
    # END PROBLEM 1
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def selecter(paragraph):
        para = split(remove_punctuation(lower(paragraph)))
        return any([word in para for word in subject])
    return selecter
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    if typed.strip() == "" and source == "":
        return 100.0
    if typed.strip() == "":
        return 0.0
    percentage = 100 / len(typed_words)
    def inside(t, s):
        if t == [] or s == []:
            return 0.0
        if t[0] == s[0]:
            return percentage + inside(t[1:], s[1:])
        else:
            return 0.0 + inside(t[1:], s[1:])
    return inside(typed_words, source_words)
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    return 0.0 if typed == '' else len(typed) / 5 / (elapsed / 60)
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    weights = [diff_function(typed_word, real_word, limit) for real_word in word_list]
    picked = word_list[weights.index(min(weights, key=abs))]
    return picked if min(weights, key=abs) <= limit and typed_word not in word_list else typed_word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    if typed[0] != source[0]:
        if limit == 0 and (typed or source):  # do not put this statement outside as a base case
            return 1
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0] == source[0]: # Feel free to remove or add additional cases
        ...
        add = 1 + minimum_mewtations(typed, source[1:], limit - 1)  # Add a character
        remove = 1 + minimum_mewtations(typed[1:], source, limit - 1)  # Remove a character
        substitute = 1 + minimum_mewtations(typed[1:], source[1:], limit - 1)  # Substitute a character
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def report_progress(typed, source, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    i, hit = 0, 0
    while i < len(typed) and i < len(source) and typed[i] == source[i]:
        i += 1
        hit += 1
    progress = hit / len(source)
...
def time_per_word(words, timestamps_per_player):
    ...
    for player in timestamps_per_player:
        values.append([y - x for x, y in zip(player, player[1:])])
    return words, values
    # END PROBLEM 9
...
def time_per_word_match(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    result = time_per_word(words, timestamps_per_player)
    return match(*result)
    # END PROBLEM 10
...
def fastest_words(match_object):
    ...
    # BEGIN PROBLEM 11
    "*** YOUR CODE HERE ***"
    fastest = [list() for _ in player_indices]
    for index in word_indices:
        min_index, min_value = float('inf'), float('inf')
        for rank in player_indices:
            if get_time(match_object, rank, index) < min_value:
                min_index, min_value = rank, get_time(match_object, rank, index)
        fastest[min_index].append(get_word(match_object, index))
    return fastest
