...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        # (define x 2) -> expressions = Pair('x', Pair(2, nil))
        # (define x (+ 7 3)) -> expressions = Pair('x', Pair(Pair('+', Pair(7, Pair(3, nil))), nil))
        # so, use expressions.rest.first to get subexpression: 2 or (+ 7 3)
        # not (2) or ((+ 7 3))
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        name, formal, body = signature.first, signature.rest, expressions.rest
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        # if false value, return it
        if is_scheme_false(value):
            return value
        expressions = expressions.rest
    # if all true, return last value
    return value
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            # Pair structure: Pair(Pair(test), Pair(sub-expr)) for one clause
            if (len(clause) == 1):
                return test
            else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    # bindings structure: Pair(Pair(name, val), bindings)
    while (bindings is not nil):
        pair0 = bindings.first  # Pair(name, val)
        validate_form(pair0, 2, 2)
        name = pair0.first
        val = scheme_eval(pair0.rest.first, env)
        names = Pair(name, names)
        vals = Pair(val, vals)
        bindings = bindings.rest
    validate_formals(names)  # all bind-name inside let should validate(no same name)
...
def do_mu_form(expressions, env):
    """Evaluate a mu form.
    Unlike lambda, mu binds its parameter dynamically, which lookups in calling frames."""
    validate_form(expressions, 2)
    formals = expressions.first
    validate_formals(formals)
    # BEGIN PROBLEM 11
