...
class Account:
    ...
    def time_to_retire(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        return next((i for i in range(1000) if self.balance * (1 + self.interest) ** i >= amount))
...
class Transaction:
    def __init__(self, id, before, after):
        self.id = id
        self.before = before
        self.after = after if after != 'Insufficient funds' else self.before
...
class BankAccount:
    ...
    def make_transaction(f):
        """A decorator that adds a transaction to the transaction history
        whenever a transaction is made.
        """
        def decorated(self, *args):
            assert isinstance(self, BankAccount), "must call on a BankAccount"
            before = self.balance
            result = f(self, *args)
            self.transactions.append(Transaction(self.len_transactions, before, result))
            return result
        return decorated
...
def make_change(amount, coins):
    ...
    if amount == smallest:
        return [smallest]
    tail = make_change(amount - smallest, rest)
    if tail != None:
        return [smallest] + tail
    else:
